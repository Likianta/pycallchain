...
class VirtualRunner:
    ...
    def run_block(self, module: str):
        ...
        if module not in self.module_linos:
            # 说明此 module 是从外部导入的模块, 如 module = 'testflight.downloader'.
            assert self.module_analyser.is_prj_module(module)
            self.outer_calls.append(module)
            # return module_path
        ...
        linos = self.module_linos[module]
        # the linos is in ordered
        lk.loga(module, linos)
    ...
    def run_line(self, lino: int):
        ...
        for i in ast_line:
            ...
            # obj_type: str. some value of it is registered in self.registered
            # _methods.keys()
            # obj_val: str/dict.
...
class ModuleAnalyser:
    ...
    def __init__(self, prjdir, pyfile, exclude_dirs=None):
        self.prjdir = file_sniffer.prettify_dir(abspath(prjdir))
        lk.loga(self.prjdir)
        # prjdir = '../' -> 'D:/myprj/'
    ...
    def get_project_modules(self, exclude_dirs=None) -> list:
        ...
        OT: prj_modules: iter. e.g. ('src.app', 'src.downloader', ...)
        """
    ...
    def indexing_module_linos(self, top_module='', linos=None):
        """
        根据 {lino:indent} 和 ast_tree 创建 {module:lino_list} 的字典.

        IN:
            ast_tree: dict. {lino: [(obj_type, obj_value), ...], ...}
                lino: int. 行号, 从 1 开始数
                obj_type: str. 对象类型, 例如 "<class 'FunctionDef'>" 等. 完整的支持
                    列表参考 src.utils.ast_helper.AstHelper#eval_node()
                obj_value: str/dict. 对象的值, 目前仅存在 str 或 dict 类型的数据.
                    示例:
                        (str) 'print'
        ...
        OT:
            module_range: dict. {module: lino_list}
                module: str. 模块的路径名
                ...
                e.g. {'src.app.module': [1, 2, 3, 9, 10],
                      ...
                      ...}
                有了 module_range 以后, 我们就可以在已知 module 的调用关系的情况下, 专注于
                读取该 module 对应的区间范围, 逐行分析每条语句, 并进一步发现新的调用关系, 以
                此产生裂变效应. 详见 TODO
        """
        lk.logd('indexing_module_linos')
        ...
        def get_obj_type():
            ...
            # ast_line is type of list, here we only take its first element to
            # see
            obj0_type, obj0_val = ast_line[0]
            return obj0_type, obj0_val
        ...
        for lino in linos:
            ...
            """
            indent >= last_indent: 在 indent_holder 中开辟新键
            indent < last_indent: 从 indent_holder 更新并移除高缩进的键
            """
            ...
            case 2:
                ...
                -> module = 'src.app.main.child_method'
            """
...
class AssignAnalyser:
    ...
    def indexing_assign_reachables(
        ...
        # 注: 为什么要这样做? 因为 ast_indents.keys() 的 linos 是不完整的, 因此要这样过
        # 滤一下.
